  ...
  return (
    <div>
        ...
        <DropdownMenu invisible={invisible}
            ...
            user={currentAccount.sellerStatus && 'Seller' + `${currentAccount.buyerStatus && ' / Buyer'}` || currentAccount.buyerStatus && 'Buyer' || 'Visitor'}
        />
    </div>
  )
}
...
const DropdownMenu = ( {invisible, info, address, user} ) => {
  ...
  return (
      <div className={`${invisible && 'hidden'} px-5 py-1 absolute bg-dimmed-black/80 rounded-md backdrop-blur-sm`}>
        ...
        <ul className='mt-2 py-2  rounded-md shadow-md text-blue-100'>
            ...
            <div className='flex gap-3'>
                <p>Wallet: </p>
                <p className='font-semibold'>{address.slice(0, 4) + '...' + address.slice(35)}</p>
